...
带有现代化Web界面的Python应用，用于连接远程SSH服务器，支持重复文件检测、批量重命名、目录整理等多种文件管理功能。

**最新更新 (v2.0)**:
- ✨ 新增功能切换面板，支持三种主要功能模式
- 🔄 批量重命名功能：支持正则表达式和多种快捷重命名方式
- 📁 目录整理功能：为文件自动创建同名文件夹并移入
- 🧹 空目录清理：自动移除空文件夹
- 🎨 优化用户界面，功能分类更清晰
- 🗑️ 移除测试脚本，保持项目整洁
...
#### 🔄 批量重命名功能
- **移除数字前缀**：自动去除文件名前的数字和空格（如 "01 文件.txt" → "文件.txt"）
- **添加序号前缀**：按顺序给文件添加编号（如 "文件.txt" → "001_文件.txt"）
- **正则表达式重命名**：支持自定义正则表达式进行高级重命名
- **快捷操作**：
  - 移除特殊字符
  - 转换大小写
  - 空格替换为下划线
...
1. 在Web界面的"SSH服务器连接"面板中输入：
   ...
   - 端口（默认：22）
2. 点击"连接"按钮
3. 连接成功后，状态指示器会变为绿色"已连接"，功能选择面板会自动显示
...
1. **选择目标目录**
   - 在"目标目录"字段输入要重命名的文件所在目录
   - 可以点击文件夹图标浏览选择

2. **选择重命名类型**
   - **移除数字前缀**：去除文件名开头的数字和空格
     - 示例：`01 照片.jpg` → `照片.jpg`
   - **添加序号前缀**：按顺序添加三位数编号
     - 示例：`照片.jpg` → `001_照片.jpg`
   - **移除特殊字符**：删除文件名中的特殊字符
     - 示例：`文件@#$.txt` → `文件.txt`
   - **转为小写/大写**：统一文件名大小写
   - **空格替换为下划线**：`文 件.txt` → `文_件.txt`
   - **正则表达式**：使用自定义正则表达式进行高级重命名

3. **配置正则表达式（如选择）**
   - 在"正则表达式模式"中输入匹配模式
   - 在"替换内容"中输入替换文本（留空表示删除）
   - 示例：
     - 模式：`^\d+\s*` 替换：`` （移除开头数字和空格）
     - 模式：`\s+` 替换：`_` （空格替换为下划线）
...
2. **选择整理类型**
   - **为文件创建同名文件夹**：
     - 为目录下的每个文件创建以文件名（不含扩展名）命名的文件夹
     - 将文件移动到对应的文件夹中
     - 示例：`文档.pdf` → `文档/文档.pdf`
   
   - **移除空目录**：
     - 扫描目录下的所有子文件夹
     - 删除所有空文件夹（不包含任何文件的文件夹）
